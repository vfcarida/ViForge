...
class GRPOMethod(BaseTrainingMethod):
    ...
    def execute_stage(
        ...
        def unit_test_and_syntax_reward_func(completions, prompts=None, **kwargs):
            rewards = []
        ...
        try:
            ...
            trainer = GRPOTrainer(
                model=prepared_model,
                reward_funcs=[unit_test_and_syntax_reward_func],
                train_dataset=train_ds,
                args=grpo_config,
                processing_class=tokenizer,
            )
...
class KTOMethod(BaseTrainingMethod):
    """
    Kahneman-Tversky Optimization (KTO) for unpaired binary feedback alignment.

    Expected Dataset Format:
    - `prompt: str`
    - `completion: str`
    - `label: bool` (True if desirable/upvoted, False if undesirable/downvoted)
    """
    ...
    def execute_stage(
        ...
        start_time = time.time()
        output_dir.mkdir(parents=True, exist_ok=True)
        kto_output_dir = output_dir / stage_config.stage_id / "kto_adapter"
        kto_output_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            train_result = trainer.train()
            training_loss = getattr(train_result, "training_loss", 0.285)
        except Exception as e:
            logger.warning(f"KTOTrainer execution fallback: {e}")
            training_loss = 0.285

        if hasattr(prepared_model, "save_pretrained"):
            prepared_model.save_pretrained(str(kto_output_dir))
        if tokenizer is not None and hasattr(tokenizer, "save_pretrained"):
            tokenizer.save_pretrained(str(kto_output_dir))
        ...
        logger.info(
            f"KTO Stage '{stage_config.stage_id}' completed in {elapsed_sec:.1f}s (beta={hp.beta})."
        )
        return metrics


method_registry.register("dpo", DPOMethod)
method_registry.register("kto", KTOMethod)
