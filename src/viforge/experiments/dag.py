...
class StageDAGResolver:
    ...
    @classmethod
    def resolve_execution_order(cls, stages: List[TrainingStageConfig]) -> List[TrainingStageConfig]:
            ...
            if s.stage_id not in visited:
                ...
                if s.depends_on:
                    if s.depends_on not in stage_map:
                        raise ValueError(f"Dependency '{s.depends_on}' not found for stage '{s.stage_id}'")
